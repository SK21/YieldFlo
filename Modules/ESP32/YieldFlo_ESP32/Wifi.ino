...
// it stays ticked until the user unticks it in the portal. What the firmware
// manages instead is how OFTEN it retries and how EXPENSIVE each attempt is.
//
// Keeping those two separate is the whole point. The old code counted
// disconnects and, at six, wrote WifiModeUseStation = false to EEPROM and
// rebooted — reading "the network is unreachable" as "the user no longer wants
// station mode", and silently discarding a setting the user had chosen. Worse,
// SaveData() writes the ENTIRE MDL struct, so a WiFi dropout also committed the
// deliberately session-only CAN→WiFi fallback in CheckCanBus() to flash.
//
...
// drivers racing, so the radio was off-channel essentially all of the time. That
// is what made the portal unusable. Pacing the attempts, and preferring the
// cheap kind, is the actual fix; setAutoReconnect(false) in StartWifi() leaves
// exactly one retry driver, which is ServiceWifiStation().
//
...
// portal that the password was refused, which is the actionable fact.
...
bool     StaForceScan = false;			// portal "Retry Network Now" pressed
...
void StartWifiAP()
{
	...
	// non-event. Falls back to 6 until a network has been seen once.
	StaChannel = MDL.StaChannelCache;
	uint8_t APchannel = (StaChannel >= 1 && StaChannel <= 13) ? StaChannel : 6;
...
{
	if (!MDL.WifiModeUseStation) return;
	...
	// invisible retry driver is half of what made the portal unusable before.
...
// Store the cached channel, and ONLY that byte. Writing the whole MDL struct
// here would re-create the bug this work removed: CheckCanBus() drops
...
void WiFiGotIP(WiFiEvent_t event, WiFiEventInfo_t info)
{
	...
	// being deaf — and StartWifi() can bring the softAP up on this channel next
	// boot, so a successful join no longer drags the softAP across channels and
	// drops every client already on it.
...
void WiFiStationDisconnected(WiFiEvent_t event, WiFiEventInfo_t info)
{
	...
	// event dispatch and nothing here touches flash. The old handler did both:
	// EEPROM.commit() followed by delay(3000) inside SaveData() stalled every
	// WiFi and IP event for three seconds.
	StaConnected = false;
	...
	// tablet sitting on the hotspot keeps receiving data through the outage.
	Wifi_DestinationIP = IPAddress(192, 168, MDL.ID + 200, 255);
}

// ── Accessors ────────────────────────────────────────────────────────────
// Wifi.ino sorts last, so the sketch preprocessor appends it after Begin.ino,
// Comm.ino and PgMain.ino. Those files can call functions declared here — the
...
// Called from the portal's real page handlers. A page load means the user is
// standing at the web UI right now, which is exactly when a connect attempt
// would be noticed.
...
// "Retry Network Now" on the portal. A full scan is the only way to find a
// network that has moved channel or been replaced, and it is also the most
// disruptive thing this radio does — so it happens when the user asks for it
// and is expecting a hiccup, rather than silently underneath them.
...
uint8_t StaCachedChannel()
{
	return StaChannel;
}

// True once the network has refused our password often enough to believe it.
// The portal says so plainly rather than leaving "not connected" to cover a
// wrong password, an absent router and a dead aerial alike.
...
void ServiceWifiStation()
{
	...
	// something, and this function runs on every loop() — roughly 1 kHz, because
	// WebServer's _nullDelay puts a delay(1) in the idle path. In particular
	...
	// A scan from the portal owns the radio until it finishes. Starting a
	// connect on top of it makes the driver abort one or the other, and the scan
	// is the one the user is standing there waiting for.
	if (WiFi.scanComplete() == WIFI_SCAN_RUNNING) return;

	// Someone loading portal pages is using the web UI right now. Suspend
	// retries outright rather than merely slowing them — the user is configuring
	// for a couple of minutes, losing retries for that window costs nothing, and
	// it removes any question of the radio competing with the page they are
	// waiting on. The portal's own retry button is the deliberate exception.
	if (!forced && LastPortalMs != 0 && (now - LastPortalMs) < PortalActiveMs) return;

	// A client associated to the hotspot but not loading pages is most likely a
	// tablet taking the data stream. It can afford a directed attempt — about
	// one dropped packet at 5 Hz — but not a 2 s scan, so stretch the interval
	// and stay cheap. This is also the normal all-day state, which is why the
	// full scan needs the earned-after-30-minutes escape below: without it,
	...
	StaAttemptPending = true;
	if (fullScan) WiFi.begin(MDL.SSID, MDL.Password);
	else          WiFi.begin(MDL.SSID, MDL.Password, StaChannel, StaHaveBssid ? StaBssid : NULL);
}
