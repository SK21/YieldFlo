...
// is byte-for-byte common with the STM32F1 module, and an app that never sees
// a PK3 (older firmware) has to keep working on the duty channel alone. Every
// field is a raw count — no calibration constant is applied here, so a stored
// log can be re-derived under any baseline afterwards.
//
...
// ── Third packet: paddle-event flow channel (5 Hz) ───────────────────────
// UDP — 11 bytes, PGN 40003
//   [0-1] PGN 40003 LE  (0x43 0x9C)
...
//   [5-6] accounted_ms uint16 LE  window time inside accepted cycles, ms
...
//   [10]  CRC8
// CAN  — ID 0x18FF02F8, DLC=8, same field layout minus the PGN header (see SendCANPK3).
//
// Receiver math (pitch-free — multiply by paddle pitch for mm/s if wanted):
//   window_s    = accounted_ms / 1000
//   paddles/s   = paddles / window_s
...
void SendUdpPK3()
{
	...
	pkt[1] = 0x9C;
	pkt[2] = flags;
	pkt[3] = (byte)(FlowSumX1000 & 0xFF);
	pkt[4] = (byte)(FlowSumX1000 >> 8);
	pkt[5] = (byte)(FlowAccountedMs & 0xFF);
	pkt[6] = (byte)(FlowAccountedMs >> 8);
	pkt[7] = FlowPaddles;
	pkt[8] = FlowRejects;
	pkt[9] = FlowSatPaddles;
	pkt[10] = CRC(pkt, 10, 0);
